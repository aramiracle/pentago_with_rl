...
import random
import numpy as np
from app.environment import PentagoEnv  # Assuming PentagoEnv is correctly implemented
...
class DDQNAgent:
    def __init__(self, env, buffer_capacity=1000000, batch_size=64, target_update_frequency=10):
        self.env = env
        self.model = DuelingDQN()  # Change here to CNN DuelingDQN
        self.target_model = DuelingDQN()  # Change here to CNN DuelingDQN
    ...
    def train_step(self):
        if len(self.buffer) >= self.batch_size:
            self.frame_idx += 1
            self.buffer.update_beta(self.frame_idx) # Update beta

            experiences, indices, weights = self.buffer.sample(self.batch_size) # Sample with weights and indices
            ...
            td_errors = 0.5 * (td_errors_board + td_errors_rotation) # Combine errors (you might need to adjust this)
...
if __name__ == '__main__':
    ...
    epsilon_final = 0.01
    epsilon_decay = 0.995
    num_episodes = 1000  # Increased episodes for better learning
    batch_size = 128       # Increased batch size
    target_update_frequency = 500 # Increased target update frequency
    buffer_capacity = 1000000

    # Re-initialize agents with new hyperparameters and CNN model
    ddqn_agents = [DDQNAgent(env, buffer_capacity=buffer_capacity, batch_size=batch_size, target_update_frequency=target_update_frequency) for _ in range(2)]
    ...
    }, 'saved_agents/ddqn_agents_trained.pth')
    print(f"Trained CNN-based DDQN agents saved to 'saved_agents/ddqn_agents_trained.pth'")