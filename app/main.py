        ...
        if ok and agent_type:
            if agent_type == "DDQN":
                if self.current_player == 1:
                    ...
                    self.load_agent('saved_agents/ddqn_agents_trained.pth', player=2)
                else:
                    self.agent = DDQNAgent(PentagoEnv())
                    self.load_agent('saved_agents/ddqn_agents_trained.pth', player=1)
            elif agent_type == "DDQN2":
                if self.current_player == 1:
                    self.agent = DDQN2Agent(PentagoEnv2())
                    self.load_agent('saved_agents/ddqn2_agents_trained.pth', player=2)
                else:
                    self.agent = DDQN2Agent(PentagoEnv2())
                    self.load_agent('saved_agents/ddqn2_agents_trained.pth', player=1)
            elif agent_type == "Hybrid":
                if self.current_player == 1:
                    self.agent = HybridAgent(PentagoEnv())
                    self.load_agent('saved_agents/hybrid_agents_trained.pth', player=2)
                else:
                    self.agent = HybridAgent(PentagoEnv())
                    self.load_agent('saved_agents/hybrid_agents_trained.pth', player=1)
            elif agent_type == "Hybrid2":
                if self.current_player == 1:
                    self.agent = Hybrid2Agent(PentagoEnv2())
                    self.load_agent('saved_agents/hybrid2_agents_trained.pth', player=2)
                else:
                    self.agent = Hybrid2Agent(PentagoEnv2())
                    self.load_agent('saved_agents/hybrid2_agents_trained.pth', player=1)
