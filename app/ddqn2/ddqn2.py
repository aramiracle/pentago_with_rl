...
class DuelingDQN(nn.Module):
    def __init__(self):
        ...
        # Advantage stream layers for combined actions
        self.fc_advantage_combined1 = nn.Linear(512, 256)
        self.bn_advantage_combined1 = nn.BatchNorm1d(256)
        self.fc_advantage_combined2 = nn.Linear(256, 36 * 8) # 36 board actions * 8 rotations
    ...
    def forward(self, x):
        ...
        # Advantage stream for combined actions
        x_advantage_combined = F.relu(self.bn_advantage_combined1(self.fc_advantage_combined1(x_shared)))
        advantage_combined = self.fc_advantage_combined2(x_advantage_combined)

        # Combine value and advantage to get Q-values for combined actions
        q_values_combined = value + (advantage_combined - advantage_combined.mean(dim=1, keepdim=True))
...
class DDQN2Agent:
    ...
    def select_action(self, state, epsilon):
        # Directly check valid actions from env
...
def agent_vs_agent_train(agents, env, num_episodes=1000, epsilon_start=0.5, epsilon_final=0.01, epsilon_decay=0.999):
    ...
    for episode in tqdm(range(num_episodes), desc="Agent vs Agent Training (CNN)", unit="episode"):
        state = env.reset()
...
if __name__ == '__main__':
    ...
    # checkpoint_path = 'saved_agents/ddqn_cnn_agents_trained.pth'
    ...
    ddqn_agents = [DDQN2Agent(env, buffer_capacity=buffer_capacity, batch_size=batch_size, target_update_frequency=target_update_frequency) for _ in range(2)]
    ...
    # Save the trained agents - save with a different name to distinguish CNN agents
    ...
    }, 'saved_agents/ddqn2_agents_trained.pth')
    print(f"Trained CNN-based DDQN2 agents saved to 'saved_agents/ddqn2_agents_trained.pth'")